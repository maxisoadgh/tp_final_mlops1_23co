...
│   ├── training.py                    # Entrenamiento LR, KNN, RF, XGBoost con Optuna
│   ├── evaluation.py                  # Métricas y selección del mejor modelo
...
### `training.py`
Funciones de entrenamiento para cada modelo, integradas con Optuna + MLflow. Usa `mlflow.start_run(nested=True)` para que cada trial de Optuna sea un run hijo visible en la UI.
